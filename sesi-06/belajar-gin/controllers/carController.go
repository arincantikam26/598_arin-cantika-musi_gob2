...
func UpdateCar(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusOK, gin.H{
		"message": fmt.Sprintf("car with id %v has been successfull updated", carID),
	})
...
func GetAllCar(ctx *gin.Context) {
	...
	for key, _ := range CarDatas {
		cars = append(cars, CarDatas[key])
	}
	if cars == nil {
		ctx.AbortWithStatusJSON(http.StatusNotFound, gin.H{
			"error_status":  "Data Not Found",
			"error_message": fmt.Sprintf("car not found"),
		})
		return
